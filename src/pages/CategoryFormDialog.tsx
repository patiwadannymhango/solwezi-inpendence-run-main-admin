...
const EMPTY: RaceCategoryPayload = {
  name: '',
  code: '',
  description: '',
  distance_label: '',
...
export default function CategoryFormDialog({
  ...
  open: boolean
  category: AdminRaceCategory | null
  onClose: () => void
  onSaved: () => void
}) {
  const isEdit = !!category

  const [form, setForm] = useState<RaceCategoryPayload>(EMPTY)
  ...
  const set = <K extends keyof RaceCategoryPayload>(key: K, value: RaceCategoryPayload[K]) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(extractErrorMessage(err, 'Could not save this race category.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>{isEdit ? 'Edit race category' : 'New race category'}</DialogTitle>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 5 }}>
              <TextField
                ...
                helperText="Used in the bulk-upload CSV's category_code column."
              />
            </Grid>
            <Grid size={12}>
              <TextField
